...
def observations_to_image(observation, info, config=None, frame_id=0, **kw):
    """Simple replacement: tile all HxWx3 uint8 sensor images side-by-side."""
    panels = []
    target_h = 256
    for key, val in observation.items():
        ...
        scale = target_h / h
        import cv2 as _cv2
        arr = _cv2.resize(arr, (max(1, int(w * scale)), target_h),
...
def play_env(env, args, config):
    ...
    if not args.no_render:
        ...
        import os as _os
        _os.environ.setdefault("SDL_RENDER_DRIVER", "software")
        pygame.init()
